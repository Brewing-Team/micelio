...
(function() {
  ...
  // Please do not edit this file directly; instead, configure the plugin in your package.json or on the quartz.config.yaml file.
  // =========================================================
  const DEFAULT_CONFIG = {
    // Visual appearance
    scalePixels: 5, // pixelation level (lower = higher resolution)
    opacity: 0.4, // canvas opacity (0.1 to 1.0)
  ...
  function updateColors() {
    ...
    // Parse hex to RGB and add alpha channel
    ...
    const auraHex = root.getPropertyValue('--lightgray').trim();
    const bodyHex = root.getPropertyValue('--tertiary').trim();
    const coreHex = root.getPropertyValue('--secondary').trim();
    
    palette = {
      aura: hexToRgba(auraHex, opacity),
      body: hexToRgba(bodyHex, opacity),
      core: hexToRgba(coreHex, opacity)
    };
  }
  ...
  function init() {
    ...
    // Inject CSS styling once
    if (!document.getElementById('micelio-bg-style')) {
      const style = document.createElement('style');
      style.id = 'micelio-bg-style';
      style.innerHTML = `
        #micelio-bg {
          position: fixed;
          top: 0;
